...
if (( $use_fork_join_check == 1)); then
  ...
  git ls-remote --heads "$url_1" $prefix_1* $prefix_2* ${prefix_victims:+${prefix_victims}*} > "$path_async_output/remote_refs_1.txt" &
  pid_remote_refs_1=$!
  git ls-remote --heads "$url_2" $prefix_1* $prefix_2* ${prefix_victims:+${prefix_victims}*} > "$path_async_output/remote_refs_2.txt" &
  pid_remote_refs_2=$!
  ...
  remote_refs_2=$(git ls-remote --heads "$url_2" $prefix_1* $prefix_2* ${prefix_victims:+${prefix_victims}*})
  remote_refs_1=$(git ls-remote --heads "$url_1" $prefix_1* $prefix_2* ${prefix_victims:+${prefix_victims}*})
fi;
