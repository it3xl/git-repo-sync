...
non_conflicting_common_branch=${prefix_victims}non-conflicting-on-client
...
if [[ ! -f "$url_1/config" ]]; then
  ...
  git add --all
  git commit -m "commit for non-conflicting-on-client file"
  ...
  git add --all
  git commit -m "commit a file in conflicting common branch on client"
  ...
  echo
  echo "@@ 2 local repo. Adding conflicting common branch created second"
  git switch $conflicting_common_branch
  echo $conflicting_common_branch on vendor >> conflicting-on-client.txt
  git add --all
  git commit -m "commit a file in conflicting common branch on vendor"
